"""Eco-GNRD + GraphLearner v2: dense init, no L1 sparsity.

Fix: A starts near 1 (fully connected), learns to prune.
No sparsity penalty — let the model decide structure freely.
"""
import numpy as np
import torch
...
def train_one_gl(visible, hidden, seed, device, cfg, n_recon_ch=None):
    ...
    for epoch in range(epochs):
        ...
        # NO sparsity penalty on GraphLearner (v2 fix)
...
def main():
    ...
    out_dir = Path("重要实验/results/graph_learner_v2")
    out_dir.mkdir(parents=True, exist_ok=True)

    # Huisman true alpha for verification
    K = np.array([
        [0.26,0.34,0.30,0.24,0.23,0.41],[0.22,0.39,0.34,0.30,0.27,0.16],
        [0.27,0.22,0.39,0.34,0.30,0.07],[0.26,0.24,0.22,0.39,0.34,0.28],
        [0.34,0.30,0.22,0.20,0.39,0.40]])
    C = np.array([
        [0.04,0.04,0.07,0.04,0.04,0.22],[0.08,0.08,0.08,0.10,0.08,0.14],
    ...
    baseline_ref = {'sp1':0.313,'sp2':0.640,'sp3':0.450,'sp4':0.591,'sp5':0.431,'sp6':0.099}
    gl_v1_ref = {'sp1':0.272,'sp2':0.391,'sp3':0.674,'sp4':0.443,'sp5':0.248,'sp6':0.584}
    ...
    for ds_name in ['huisman', 'beninca', 'maizuru']:
        ...
        print(f"\n{'='*60}")
        print(f"GraphLearner v2 (dense init, no L1) on {ds_name}")
        print(f"{'='*60}")
        ...
        for vis, hid, sp_name, n_rc in tasks:
            ...
            for seed in SEEDS_TEST:
                ri += 1
                t0 = datetime.now()
                r_val, A = train_one_gl(vis, hid, seed, device, cfg, n_recon_ch=n_rc)
            ...
            A_info = ""
            if sp_As and ds_name == 'huisman':
                A_mean = np.mean(sp_As, axis=0)
                h_idx = SP.index(sp_name)
                vis_idx = [i for i in range(6) if i != h_idx]
                A_sp = A_mean[:5, :5]
                true_sp = alpha[np.ix_(vis_idx, vis_idx)]
                sr, _ = spearmanr(A_sp.flatten(), true_sp.flatten())
                A_info = f"  Spearman={sr:+.3f}, A_mean={A_mean.mean():.3f}"
        ...
        with open(out_dir / f"{ds_name}_summary.md", 'w', encoding='utf-8') as f:
            f.write(f"# {ds_name}: GraphLearner v2\n\n")
            f.write("| Species | GL_v2 | Baseline | GL_v1 |\n|---|---|---|---|\n")
            for r in results:
                bv = baseline_ref.get(r['species'], 0) if ds_name == 'huisman' else 0
                gv1 = gl_v1_ref.get(r['species'], 0) if ds_name == 'huisman' else 0
                f.write(f"| {r['species']} | {r['mean_val']:+.3f} | {bv:+.3f} | {gv1:+.3f} |\n")
            f.write(f"\n**Overall**: {ov:+.3f}\n")
