...
SEEDS_TEST = [83781, 25836, 36228, 41060, 90950]
LAM_SPARSE_GL = 0.01  # v1=0.01 (too much), v2=0 (too little), v3=0.001
...
def make_model_gl(N, cfg, device):
    return CVHI_Residual(
        ...
        use_G_field=True, use_graph_learner=True, gl_renorm_strength=0.3, gl_replace_attn=False,
...
def train_one(visible, hidden, seed, device, cfg, n_recon_ch=None):
    ...
    for epoch in range(epochs):
        ...
        # Lambda annealing: ramp L0 penalty from 0 to target over first 200 epochs
        # Only start after warmup (epoch 100), so structure learning begins after dynamics stabilize
        if epoch < warmup:
            lam_gl = 0.0
        else:
            lam_gl = LAM_SPARSE_GL * min(1.0, (epoch - warmup) / 200.0)
    ...
    if np.any(np.isnan(h_mean)) or np.std(h_mean) < 1e-10:
        r_val = 0.0
    else:
        try:
...
def main():
    ...
    out_dir = Path("runs/graph_learner_v12")
    ...
    for ds_name in ['huisman', 'beninca', 'maizuru']:
        ...
        for vis, hid, sp_name, n_rc in tasks:
            ...
            for seed in SEEDS_TEST:
                ...
                # Save per-seed metrics immediately
                seed_dir = out_dir / ds_name / sp_name / f"seed_{seed:05d}"
                seed_dir.mkdir(parents=True, exist_ok=True)
                import json
                with open(seed_dir / "metrics.json", 'w') as mf:
                    json.dump({'species': sp_name, 'seed': seed, 'pearson_val': r_val, 'dataset': ds_name}, mf)
                sys.stdout.flush()
